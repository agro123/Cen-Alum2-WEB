...
const OrdenServicioEditar = () => {
  const validar = () => {
    if (data.ordenServicio[parseInt(data.indice) - 1] === undefined)
      return ordenDeServicio
    else return ordenDeServicio = data.ordenServicio[parseInt(data.indice) - 1]
  }
  validar();
  ...
  const [client, setClient] = useState(ordenDeServicio.cliente);
  const [materials, setMaterials] = useState([]);
  const [otrosGastos, setOtrosGastos] = useState([]);
  const [responsables, setResponsables] = useState([]);
  const [observation, setObservation] = useState(ordenDeServicio.observaciones);
  const [date, setDate] = useState(ordenDeServicio.fechaFinal);
  const [numCotizacion, setNumCotizacion] = useState(ordenDeServicio.cotizacionNum)
    ...
    <>
      <div className='top'>
        ...
        <ShowDate handleDate={handleDate}
                  inputDate= {ordenDeServicio.fechaFinal} />
      </div>
